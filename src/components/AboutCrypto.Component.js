...
const AboutCryptoComponent = () => {
    const theme = useSelector((state) => state.theme.value)
    const themeText = theme === 'dark' ? 'light' : 'dark';
    return (
        <div className={`card text-${themeText} bg-${theme}`}>
            <h1 className='card-header'>What is Cryptocurrency ?</h1>
            ...
            <p>The first decentralized cryptocurrency was Bitcoin, which was first released as open-source software in 2009. As of March 2022, there were more than 9,000 other cryptocurrencies in the marketplace, of which more than 70 had a market capitalization exceeding $1 billion.</p>
            <p><small>Font: <Link to="https://en.wikipedia.org/wiki/Cryptocurrency" target={'_blank'}>Wikipedia</Link></small></p>
        </div>
    );
}

export default AboutCryptoComponent;
